...
class MintTest < Minitest::Test
  ...
  def test_register
    ...
    assert_equal Mint.currency_for('SGX'), sgx
  end
  ...
  def test_zero_with_currency_object
    assert_equal Mint.money(0, 'USD'), Mint.zero(Mint.currency_for('USD'))
  end
  ...
  def test_concurrent_register
    ...
    codes.each { |code| refute_nil Mint.currency_for(code) }
  end
